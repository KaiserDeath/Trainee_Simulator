...
test('real local Add Credits lifecycle, queue, trainer reads, and cleanup', async ({
  ...
  await acceptDialog.getByRole('button', {
    name: 'Accept'
  }).click();
  expect(
  ...
  await cancelDialog.getByLabel(
    'Enter the reason it was canceled'
  ).fill('  Duplicate request  ');
  await cancelDialog.getByRole('button', {
    name: 'Notify'
  }).click();
  expect(
  ...
  expect(cancelledOperation.cancellation_reason)
