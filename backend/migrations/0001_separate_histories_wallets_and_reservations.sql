...
-- Existing sandbox sessions are disposable, but preserve recognizable game
-- rows while separating them from Backend customer movement history.
...
LEFT JOIN LATERAL (
  ...
  WHERE candidate.session_id = history.session_id
    ...
    AND history.description ILIKE '%' || candidate.game || '%'
...
DELETE FROM sandbox_transaction_history
